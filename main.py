    ...
    if tick % 3 == 0:
        ...
        for p in range(2*num_players):
            for q in range(2*num_players):
                if game.players[p].role == game.players[q].role:
                    continue
                for i in range(N):
                    for j in range(N):
                        if game.players[p].role == "offender":
                            A[p, q, i, j] = EvaluateTrajectories(traj_list[p][i], traj_list[q][j], aggressive_coef=1)
                        else:
                            A[p, q, i, j] = -EvaluateTrajectories(traj_list[p][i], traj_list[q][j], aggressive_coef=0)

        # Calculate probability of actions between each pair of players
        probs = np.zeros((2*num_players, N))
        for p in range(2*num_players):
            for q in range(2*num_players):
                if game.players[p].role == 'offender' and game.players[q].role == 'defender':
