...
def make_data(data_root):
    ...
    for root in os.listdir(data_root):
        for fname in os.listdir(os.path.join(data_root, root, 'after_passing')):
            data_item = np.load(os.path.join(data_root, root, 'after_passing', fname))
            if data_ap is None:
                data_ap = data_item
            else:
    ...
    data_bp = None
    for root in os.listdir(data_root):
        for fname in os.listdir(os.path.join(data_root, root, 'before_passing')):
            data_item = np.load(os.path.join(data_root, root, 'before_passing', fname))
            if data_bp is None:
                data_bp = data_item
            else:
    ...
    print(data_ap.shape)
    print(data_bp.shape)
    os.makedirs(os.path.join(data_root, 'synthesized'), exist_ok=True)
    np.save(os.path.join(data_root, 'synthesized', 'data_after_passing.npy'), data_ap)
    np.save(os.path.join(data_root, 'synthesized', 'data_before_passing.npy'), data_bp)
